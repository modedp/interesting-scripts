...
wait(0.5)
notify("PRESS [Z] TO EQUIP KNIFE || Converted by Tescalus#0001",true)
...
function kysnigga()
	...
	if me:FindFirstChildOfClass('Humanoid') then
		me:FindFirstChildOfClass('Humanoid').Health = 100
	end
	wait(0.01)
...
function bleedout()
	local doit = coroutine.wrap(function()
		...
		while targe and targe:FindFirstChildOfClass('Humanoid') and targe:FindFirstChildOfClass('Humanoid').Health > 0 and num < 11 do
			...
			targe:FindFirstChildOfClass('Humanoid').Health = targe:FindFirstChildOfClass('Humanoid').Health - 0
			num = num+1
			wait(0.325)
		end
		targe:FindFirstChildOfClass('Humanoid').Health = 100
		wait()
...
function throw()
	if grabbed.Torso:FindFirstChild("TargetWeld") == nil or grabbed:FindFirstChildOfClass('Humanoid').Health <= 100 then return end
	paralyzed = false
	targetweld = grabbed.Torso.TargetWeld
...
knife.Touched:connect(function(hitz)
	if hitz.Parent and hitz.Parent:FindFirstChildOfClass("Humanoid") and hitz.Parent:FindFirstChild('Torso') and acting == true then
		...
		elseif mode == "finish" and finishing == true then
			...
			audio:Play()
			if hit == false then
				hitz.Parent:FindFirstChildOfClass('Humanoid').Health = 100
				wait()
