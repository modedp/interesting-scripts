...
local hi= [[CLOVR - FE FULL-BODY VR SCRIPT
...
local VirtualRig = script.Dummy:Clone()
local VirtualBody = script.Mover:Clone()
...
do
	...
	if VRReady then
		local Pointer = script.VRPointer:Clone()
...
ChatHUDFunc = function()
	...
	local ChatHUD = script:WaitForChild("ChatHUD"):Clone()
...
ViewHUDFunc = function()
	...
	local ViewHUD = script:WaitForChild("ViewHUD"):Clone() or script:WaitForChild("ViewHUD"):Clone()
	 local Viewport = ViewHUD.Viewport
	  local Viewcam = Instance.new("Camera")
	 local ViewPart = ViewHUD.Part
	
	ViewHUD.Parent = plrGui
...
TrackerArm.CFrame = game.Players.LocalPlayer.Character:FindFirstChild("Right Arm").CFrame
end
wait()
end)
