local victimName = "" --[[Full username pls :) ]]
function hellDoor(username)
...
local function MoveSpawn(obj1,PLAYER)
	local playerpos = nil
...
local function MovePartBy(part,AMNT)
	part.CFrame = part.CFrame+AMNT
end

if plr.Character then
	local function thatstuff()
...
task.spawn(function() hellDoor(game.Players:GetPlayers()[i].Name) end)
end
else 
hellDoor(victimName) 
end
